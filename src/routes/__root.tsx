      ...
      <div className="error-screen">
        <div className="error-panel">
          ...
          <div className="error-actions">              <Link to="/" className="btn-game primary">
              🏠 Kembali ke Simulator Jemparingan
            </Link>
          </div>
        </div>
      </div>
...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  useEffect(() => {
    console.error("[AR Jemparingan] Root error:", error);
  }, [error]);
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=device-width, initial-scale=1" },      { title: "AR Jemparingan — Simulator Gerak Parabola" },
      {
        name: "description",
        content:
          "Simulator 3D gaya game untuk gerak parabola berbasis permainan tradisional Jemparingan.",
      },
      { property: "og:title", content: "AR Jemparingan — Simulator Gerak Parabola" },
      { property: "og:description", content:
          "Simulator 3D interaktif untuk memvisualisasikan lintasan parabola pada permainan Jemparingan.",
      },
